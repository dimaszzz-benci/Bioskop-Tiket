...
def get_db():
    if not os.path.exists(DB):
        json.dump({"users":[],"bookings":[],"movies":[],
                   "admin":{"user":"admin","pass":hashlib.md5(b"admin123").hexdigest()}}, open(DB,'w'))
...
def booking(mid):
    ...
    if request.method == 'POST':
        ...
        bk = {"id":f"TK{uuid.uuid4().hex[:8]}","user":session['user'],"movie":m['title'],
    ...
    return render_template('booking.html', movie=m, cinemas=["CGV","XXI","Cinepolis"],                           dates=["2026-04-15","2026-04-16","2026-04-17"],
...
def admin():
    if not admin_ok():
        if request.method=='POST':
            ...
            flash('Kredensial salah', 'danger')        return render_template('admin/dashboard.html', admin_login=True)
    db = get_db()
    return render_template('admin/dashboard.html', admin_login=False, bookings=db['bookings'], users=db['users'], movies=db['movies'])
